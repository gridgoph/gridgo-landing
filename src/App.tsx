...
import { Menu, X, MessageCircle, Zap, ShieldCheck, ChevronUp, Smartphone } from 'lucide-react';
import { HardDriveUploadIcon, TruckIcon, ListIcon, TimerIcon, MessageCircleIcon } from 'lucide-animated';
import { Link } from 'react-router-dom';
import { GRIDGO_APPS, landingLinks, supportTicketsEnabled } from './utils/landingLinks';
    ...
    <section className="h-[100vh] flex flex-col items-center justify-end pb-32 relative z-10 bg-map">
      ...
      >
        ...
        <p className="text-lg text-gray-300">
          Send your artwork from the app. GRIDGO matches it to a verified print partner, and a rider brings the finished job to your door so you don't have to leave your seat.
        </p>
      </motion.div>
    </section>
  );
      ...
      <div className="max-w-6xl mx-auto px-4 w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-4 mb-10">
          ...
          <FeatureCard
            ...
            desc="Follow your project through every stage it passes — artwork review, printing, packaging and quality check, then the rider on the way. You see each step as it happens, in the app."
          />
          <FeatureCard
            icon={ListIcon}
            title="The Price You Agreed To"
            desc="You see a price range the moment you place the order. When a print partner accepts it, GRIDGO tells you the final price before a single peso is asked for."
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 max-w-4xl mx-auto">
          <FeatureCard
            icon={TimerIcon}
            title="Submit Any Time"
            desc="Inspiration doesn't have a closing time. Whether it's 2 PM or 2 AM, you can upload your files and start the process. Your request is queued for review the moment it lands, so it never waits for a shop to open."
          />
          <FeatureCard
            icon={MessageCircleIcon}
            title="Support That Answers"
            desc="From upload to doorstep, help is a tap away. Raise an issue on an order and the GRIDGO team picks it up — real people, one tracked thread, no bots in the middle."
          />
        </div>
      </div>
    </section>
...
const PAYMENT_STEPS = [
  {
    title: 'Sign up as you are',
    body: 'Clients sign themselves up as a person, a business or an organization. Print partners and riders sign up too, and are profiled and approved before GRIDGO sends them any work.',
  },
  {
    title: 'A price range first, a final price second',
    body: 'You see a range when you place the order. The exact figure is only known once a print partner accepts, and GRIDGO tells you before asking for anything.',
  },
  {
    title: '75% down, 25% on the balance',
    body: 'Pay 75% by QR through GCash, Maya or another e-wallet once you have the final price. The remaining 25% is settled the same way at the end — payment is digital throughout, so riders never handle your money.',
  },
  {
    title: 'Delivery priced by distance',
    body: 'The delivery fee follows the distance between your print partner and your address, shown on your order beside the subtotal. Nothing is hidden inside the item price.',
  },
  {
    title: 'Partners are paid on proof',
    body: 'A print partner is paid across four milestones — printing, packaging and quality check, delivery, and retention after the issue window. Each one needs a Proof of Fulfilment before it releases.',
  },
  {
    title: 'An issue window that closes',
    body: 'Delivery opens a window for you to raise a problem. Raise one and the payout is held until it is settled. Raise nothing and the order completes when the window runs out.',
  },
];

const PICKUP_CHECKS = [
  ['Quantity match', 'The physical count matches the order ticket.'],
  ['Specification match', 'Size, colour, material and design match what you asked for.'],
  ['Visible defects', 'No tears, misprints, colour shifts, cracks or stains.'],
  ['Packaging integrity', 'Wrapped or boxed well enough to survive a motorcycle.'],
  ['Documentation', 'Receipt or order slip present, and matching the order ID.'],
  ['Partner sign-off', 'The print partner confirms the handoff.'],
];

function HowItReallyWorks() {
  return (
    <div className="mt-32">
      <motion.div
        initial={{ opacity: 0, y: 30, filter: 'blur(15px)' }}
      ...
      >
        <p className="text-xs tracking-[0.35em] uppercase text-[var(--color-primary)] mb-3">The details</p>
        <h3 className="text-3xl md:text-4xl font-bold">How GRIDGO actually works</h3>
      </motion.div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {PAYMENT_STEPS.map(({ title, body }, i) => (
          <motion.div
            key={title}
          ...
          >
            <h4 className="font-bold text-[var(--color-primary)] mb-2 text-base">{title}</h4>
      ...
      <motion.div
        ...
        viewport={{ once: true, margin: '-80px' }}
        className="mt-8 rounded-3xl border border-[var(--color-primary)]/20 bg-[#0A0A0A] p-8 md:p-12"
      >
        <div className="flex items-center gap-3 mb-3">
          <ShieldCheck size={26} className="text-[var(--color-primary)] shrink-0" strokeWidth={1.5} />
          <h4 className="text-2xl md:text-3xl font-bold">Checked before it moves</h4>
        </div>
        <p className="text-sm md:text-base text-gray-400 leading-relaxed max-w-3xl mb-8">
          A GRIDGO rider cannot start carrying your order until six checks pass at the print partner's counter.
          If any one of them fails, the rider does not transport it — the problem is photographed and escalated
          on the spot. A defect that leaves the counter unlogged becomes GRIDGO's problem instead of yours,
          and that is what protects the Zero-Risk Reprint Guarantee.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-5">
          {PICKUP_CHECKS.map(([title, body], i) => (
            <div key={title} className="flex gap-3">
              <span className="text-[var(--color-primary)] font-bold text-sm tabular-nums pt-0.5 shrink-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-16 text-center">
          ...
          >
            ...
            <p className="text-sm text-gray-300 leading-relaxed max-w-sm mx-auto">
              Sign up, pick a category and describe the job, then upload your artwork straight from your phone or cloud storage. You see a price range before you submit.
            </p>
          </motion.div>
          ...
          >
            ...
            <p className="text-sm text-gray-300 leading-relaxed max-w-sm mx-auto">
              GRIDGO checks your artwork, then assigns a verified print partner who works in that category. When they accept, you are told the final price and pay the downpayment.
            </p>
          </motion.div>
          ...
          >
            ...
            <p className="text-sm text-gray-300 leading-relaxed max-w-sm mx-auto">
              A GRIDGO rider inspects the job at pickup, carries it, and hands it over with photo or signature evidence. You settle the balance and the order closes.
            </p>
          </motion.div>
  ...
  return (
    <section id="support" className="min-h-screen bg-map py-24 relative z-10">
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <div className="relative z-10 max-w-xl">
            ...
            <p className="text-gray-400 leading-relaxed text-base md:text-lg">
              Human-led support, start to finish. Submit a ticket with your order issue, delivery question or account concern, and the GRIDGO admin team picks it up and replies to the email address you gave us.
            </p>
          </div>
          ...
          <Link to="/support" className="mt-4 w-full sm:w-auto px-10 py-3.5 rounded-full bg-[var(--color-primary)] text-black font-bold text-base hover:bg-[#FFE57F] hover:scale-105 hover:shadow-[0_0_25px_rgba(255,222,88,0.3)] transition-all duration-300 relative z-10">
            {supportTicketsEnabled() ? 'Submit a Ticket Now' : 'See Support Options'}
          </Link>

          {!supportTicketsEnabled() && (
            <p className="text-xs text-gray-500 relative z-10 max-w-md">
              Ticket intake opens with the public launch — the page will tell you where things stand today.
            </p>
          )}
        </motion.div>
        ...
        >
          {[
            { value: 'Human', label: 'Read by the team' },
            { value: 'One thread', label: 'Orders, delivery, account' },
            { value: 'By email', label: 'Replies to your inbox' },
          ].map(({ value, label }) => (
...
function AppDownloadCard({ app }: { app: GridgoApp }) {
  return (
    <Link
      to="/download"
      className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-left flex flex-col gap-1 h-full hover:border-[var(--color-primary)]/40 hover:bg-[var(--color-primary)]/5 transition-colors"
    >
      <span className="text-[10px] uppercase tracking-[0.2em] text-gray-400">{app.audience}</span>
      <span className="font-bold text-[15px] break-words">{app.name}</span>
      <span className="text-[12px] text-gray-400 leading-relaxed">{app.blurb}</span>
      <span className="mt-2 text-[11px] font-semibold text-[var(--color-primary)]">
        Get {app.slug} →
      </span>
    </Link>
  );
    ...
    <section id="download" className="min-h-[100vh] flex items-center justify-center relative z-10 py-20 px-8 overflow-hidden bg-[#050505]">
      ...
      <div className="max-w-4xl mx-auto w-full flex justify-end relative z-20">
        ...
        <motion.div
          ...
          viewport={{ once: true, margin: '-100px' }}
          className="w-full md:w-[55%] flex flex-col items-center text-center"
        >
          <div className="w-full max-w-[460px] flex flex-col items-center">
            ...
            <p className="text-gray-200 text-[13px] md:text-[14px] mb-8 max-w-[380px] font-light leading-relaxed">
              Become a founding member of GRIDGO and help shape the platform — as a customer, a <span className="text-[var(--color-primary)] font-medium">print partner</span>, or a rider.
            </p>

            <div className="grid grid-cols-1 gap-3 mb-6 w-full">
              {GRIDGO_APPS.map((app) => (
                <AppDownloadCard key={app.key} app={app} />
              ))}
            </div>

            <div className="flex items-center justify-center gap-2 text-[11px] text-gray-400 mb-8 px-3 py-2 rounded-full border border-white/10 bg-white/5">
              <Smartphone size={14} className="text-[var(--color-primary)] shrink-0" />
              <span>
                Android only, installed from a download here. GRIDGO has no Google Play or App Store listing.
              </span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-10 w-full justify-center">
              <Link
                to="/download"
                className="bg-[var(--color-primary)] text-black font-bold py-3 px-6 rounded-full hover:bg-[#FFE57F] transition-colors text-[13px] md:text-[14px]"
              >
                Download Apps
              </Link>
              <a
                ...
                className="bg-white text-black font-bold py-3 px-6 rounded-full hover:bg-gray-200 transition-colors text-[13px] md:text-[14px]"
              >
                Partner Dashboard
              </a>
              {communityUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="border border-[var(--color-primary)] text-[var(--color-primary)] font-bold py-3 px-6 rounded-full hover:bg-[var(--color-primary)] hover:text-black transition-colors text-[13px] md:text-[14px]"
                >
                  Join GRID Community
                </a>
              )}
            ...
            <div className="flex flex-col items-center w-full">
              ...
              <p className="font-heading text-[16px] md:text-[18px] text-white tracking-wide font-normal">Launching soon.</p>
            </div>
          </div>
        </motion.div>
  ...
  return (
    <div className="w-full relative custom-scrollbar bg-black text-white">
      ...
      {/*
        ...
        It needs its own Suspense boundary: the GLB and the drei environment
        map suspend while they load, and without a boundary here that
        suspension reaches the router's boundary and hides the whole page —
        navbar, copy and all — behind a blank screen until the 3D is ready.
        Falling back to null lets the page render immediately and the scene
        fade in when it has loaded.
      */}
