...
function PhoneRig() {
  ...
  const [, setMobile] = React.useState(window.innerWidth < 768)
...
class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  render() {
    if (this.state.hasError) {
      // This scene is decorative, and its environment map comes from a CDN that
      // can be slow, blocked or down. Rendering the raw error put a red string
      // over the page and, because it was absolutely positioned and unclipped,
      // scrolled the whole site sideways on a phone. Fail quietly instead —
      // the page reads fine without the 3D.
      console.error('GRIDGO 3D scene failed to load:', this.state.error)
      return null
    }
